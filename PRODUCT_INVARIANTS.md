...
- RX-064 implements the next concrete local/manual/fake-money paper testing-support handoff after RX-063 finalization: a local display command preview builder. It consumes one explicit local display payload fixture path, validates that fixture through the accepted RX-062 parser without reading or rendering the referenced report JSON, and writes one explicit local preview/manifest artifact with only the display payload path and the exact manual `render-paper-session-report-from-payload --paper-session-display-command-payload-json-path ...` command plan. It must not render through Telegram, send messages, call networks, use credentials, run sessions, construct adapters, write ledgers, mutate report results, discover/rank/watchlist/poll, automate execution, add live/order/private/account scope, calculate aggregate PnL, turn unknowns into zero, or create second owner paths.
- RX-065 implements the next concrete local/manual/fake-money paper testing-support handoff after RX-064 finalization: a local paper session display command text parser for later Telegram-style command interface testing without Telegram transport or credentials. It consumes one explicit command text fixture path and one explicit display payload output path, accepts exactly `paper-session-report-display --session-report-json-path <session-report-json-path>` using `shlex.split()`, validates the generated payload through the accepted RX-062 parser, and writes only `schema_version=1` plus `session_report_json_path`. It must stay local command-text parsing only and must not render through Telegram, send messages, call networks, use credentials, read or render reports, run sessions, construct adapters, write ledgers, mutate report results, discover/rank/watchlist/poll, automate execution, add live/order/private/account scope, calculate aggregate PnL, turn unknowns into zero, or create second owner paths.
- RX-066 implements the next concrete local/manual/fake-money paper testing-support handoff after RX-065 finalization: a local paper session display command text preview manifest for later Telegram-style command interface testing without Telegram transport or credentials. It stays local command-text preview only, validates through the accepted RX-065 and RX-062 boundaries, writes only one descriptive preview/manifest, and must not write display payloads, render through Telegram, send messages, call networks, use credentials, read or render reports, run sessions, construct adapters, write ledgers, mutate report results, discover/rank/watchlist/poll, automate execution, add live/order/private/account scope, calculate aggregate PnL, turn unknowns into zero, or create second owner paths.
- RX-067 is prepared as the next concrete fake-money paper-trader runtime/testability handoff after RX-066 finalization: deterministic local smoke fixture coverage for the existing `paper-trade-session` flow. It must use injected local test fixtures, preserve the existing session/decision/paper lifecycle/ledger/report owner paths, avoid new commands and CLI behavior changes, and must not call networks, use credentials, add Telegram transport, discover/rank/watchlist/poll, automate execution, add live/order/private/account scope, calculate aggregate PnL, turn unknowns into zero, or create second owner paths.
