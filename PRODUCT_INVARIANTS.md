...
- A later manual public paper-trader bridge may be implemented only by an exact current task. It may connect one existing public one-route real-data ENTRY decision to the existing fake paper lifecycle and existing append-only ledger, but must not add discovery, ranking, watchlists, polling, background loops, execution planning, guarded live runner execution, approval-boundary execution, new route statuses, new reject reasons, new decision/snapshot/EV paths, or unknown-to-zero behavior.
