...
def test_paper_session_run_command_text_parser_output_feeds_runtime_report_and_display_smoke(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
    tmp_path,
) -> None:
    routes_output_path = tmp_path / "parsed runtime routes.json"
    package_preview_path = tmp_path / "parsed-runtime-package-preview.json"
    report_path = tmp_path / "parsed runtime report.json"
    ledger_path = tmp_path / "parsed-runtime-ledger.sqlite"
    command_payload_path = tmp_path / "parsed-runtime-command-payload.json"
    command_text_path = tmp_path / "parsed runtime command.txt"
    adapter_calls = _install_deterministic_public_adapters(monkeypatch)

    routes = [
        _paper_session_route(
            route_id="parsed-runtime-started",
            capture_id="capture-parsed-runtime-started",
        ),
        _paper_session_route(
            route_id="parsed-runtime-unknown",
            capture_id="capture-parsed-runtime-unknown",
            risex_symbol="ETH-PERP",
            hedge_symbol="ETH",
            assembled_at="2026-08-13T12:01:00+00:00",
        ),
    ...
    command_text = (
        ...
        f"{shlex.quote(str(command_payload_path))} --routes-json-output-path "
        f"{shlex.quote(str(routes_output_path))} --preview-json-output-path "
        f"{shlex.quote(str(package_preview_path))} --session-report-json-path "
    ...
    def recording_read_text(self, *args: object, **kwargs: object) -> str:
        ...
        if str(self) in {
            ...
            raise AssertionError(
                "run command text parser must not read package/runtime outputs"
            )
    ...
    with monkeypatch.context() as parser_boundary:
        parser_boundary.setattr(cli.Path, "read_text", recording_read_text)
        parser_boundary.setattr(
            cli,
            "_write_json_artifact",
            recording_write_json_artifact,
        )
        assert (
            cli.main(
                [
    ...
    parser_output = capsys.readouterr().out
    assert parser_output == (
        "Paper Session Run Command Text Parser\n"
        f"command_text_path={command_text_path}\n"
        f"paper_session_command_payload_json_path={command_payload_path}\n"
        "route_count=2\n"
        "route_ids=parsed-runtime-started,parsed-runtime-unknown\n"
        f"routes_json_path={routes_output_path}\n"
        f"preview_json_path={package_preview_path}\n"
        f"session_report_json_path={report_path}\n"
    )
    assert read_paths == [str(command_text_path), str(command_payload_path)]
    assert write_paths == [str(routes_output_path), str(package_preview_path)]
    assert adapter_calls == []
    assert not report_path.exists()
    assert not ledger_path.exists()
    ...
    assert [route.route_id for route, _assembled_at in parsed_generated_routes] == [
        "parsed-runtime-started",
        "parsed-runtime-unknown",
    ]
    ...
    started_route, unknown_route = report["routes"]
    assert started_route["route"]["route_id"] == "parsed-runtime-started"
    assert started_route["decision"]["status"] == "PAPER_ELIGIBLE"
    assert started_route["decision"]["net_profit_usd"] == "7.0000"
    assert started_route["decision"]["entry_ev"] == {
        "expected_funding_usd": "8.000",
    ...
    assert unknown_route["route"]["route_id"] == "parsed-runtime-unknown"
    assert unknown_route["decision"]["status"] == "REJECTED"
    assert unknown_route["decision"]["net_profit_usd"] is None
    assert unknown_route["decision"]["entry_ev"] == {
        "expected_funding_usd": None,
    ...
    assert display_output == (
        ...
        "route_count=2\n"
        "route_ids=parsed-runtime-started,parsed-runtime-unknown\n"
        "route.1.route_id=parsed-runtime-started\n"
        "route.1.decision_status=PAPER_ELIGIBLE\n"
        "route.1.paper_started=true\n"
        "route.1.decision_net_profit_usd=7.0000\n"
        "route.1.decision_entry_ev_expected_funding_usd=8.000\n"
        ...
        "route.1.paper_simulated_roundtrip_cost_usd=0\n"
        "route.1.paper_net_profit_usd=7.0000\n"
        "route.2.route_id=parsed-runtime-unknown\n"
        "route.2.decision_status=REJECTED\n"
        "route.2.paper_started=false\n"
        "route.2.decision_net_profit_usd=null\n"
        "route.2.decision_entry_ev_expected_funding_usd=null\n"
