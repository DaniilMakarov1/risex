...
- RX-072 disposition: adds one local-only, manually invoked `build-paper-session-run-command-text-preview` CLI command plus one side-effect-free CLI app-layer parser helper for exact `paper-session-run ...` command text. The command reads only the command text fixture and referenced command payload fixture, validates the referenced payload through the accepted RX-058 parser boundary, rejects normalized local output path collisions among the new run-command preview output and referenced intended package/session output paths, and writes only one descriptive run-command-text preview/manifest artifact for the exact manual `build-paper-session-package ...` command plan.
