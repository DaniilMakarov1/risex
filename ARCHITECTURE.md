...
After RX-068 reviewer acceptance and finalization, the next handoff is RX-069 Local Paper Session End-To-End Operator Display Smoke Fixture Coverage. It is a concrete test-only/local/manual/fake-money smoke handoff over the accepted package, serial runtime, report export, display payload, display preview, command-text preview/parser, and payload-backed render command paths. It must reuse accepted commands and deterministic adapter doubles without adding new commands, production behavior, Telegram transport, network or credentials, live/order/private/account scope, aggregate PnL calculation, unknown-to-zero behavior, or second owner paths.
