...
RX-072 adds that handoff as the explicit `build-paper-session-run-command-text-preview` CLI command plus one side-effect-free parser helper in `apps/cli/paper_session_payloads.py`. The parser accepts exactly `paper-session-run --paper-session-command-payload-json-path <payload-json-path> --routes-json-output-path <routes-json-output-path> --preview-json-output-path <package-preview-json-output-path> --session-report-json-path <session-report-json-path>` through `shlex.split()` and returns only the four local path strings needed by the accepted package builder. The command reads only the command text fixture and referenced command payload fixture, validates the payload through the accepted RX-058 route-list boundary, rejects local output path collisions, and writes only one descriptive preview/manifest for the exact manual `build-paper-session-package ...` command plan. It does not run sessions, write route-list/package-preview/session-report artifacts, render displays, construct adapters, instantiate ledgers, call networks, send messages, add Telegram transport or credentials, automate execution, enter live/order/private/account scope, calculate aggregate PnL, turn unknowns into zero, or create second owner paths.
