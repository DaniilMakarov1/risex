...
- Local paper session command payload fixture parsing and route-list validation for `paper-trade-session` happen only in the CLI app layer, with `apps/cli/paper_session_payloads.py` owning side-effect-free payload normalization and validation helpers.
...
RX-062 is prepared as the next concrete local/manual/fake-money testing-support handoff: a CLI-layer parser/fixture helper for display command payloads that point at already-written RX-057 session report JSON artifacts and feed the RX-061 renderer contract. It must remain local-only and report-display-only, without Telegram transport, credentials, messaging/network behavior, session execution, adapter construction, ledger writes, report mutation, execution automation, discovery/ranking/polling, live/order/private/account scope, aggregate PnL calculation, unknown-to-zero behavior, or second owner paths.
