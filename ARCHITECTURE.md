...
- Local paper session command payload fixture parsing, route-list validation for `paper-trade-session`, run command-text parsing for package-builder previews, and display command payload parsing for `render-paper-session-report` happen only in the CLI app layer, with `apps/cli/paper_session_payloads.py` owning side-effect-free payload normalization and validation helpers.
