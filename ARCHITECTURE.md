...
RX-067 is prepared as the next concrete fake-money paper-trader runtime/testability handoff after RX-066 finalization: deterministic local smoke fixture coverage for the existing `paper-trade-session` flow. It must exercise the accepted serial session, paper lifecycle, ledger, stdout, and report-export behavior through injected local test fixtures without adding new commands, changing CLI behavior, calling networks, adding credentials, adding Telegram transport, entering live/order/private/account scope, calculating aggregate PnL, turning unknowns into zero, or creating second owner paths.
