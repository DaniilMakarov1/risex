...
RX-065 is prepared as the next concrete local/manual/fake-money testing-support handoff after RX-064 finalization: a local paper session display command text parser for later Telegram-style command interface testing without Telegram transport or credentials. It must remain local command-text parsing only, reuse accepted display payload/display command boundaries, and avoid report rendering, session execution, adapter construction, ledger writes, report/result mutation, messaging/network behavior, execution automation, discovery/ranking/polling, live/order/private/account scope, aggregate PnL calculation, unknown-to-zero behavior, or second owner paths.
