...
After RX-067 reviewer acceptance and finalization, RX-068 Local Paper Session Package-To-Runtime Smoke Fixture Coverage is the next concrete local/manual/fake-money testability handoff. It should prove that the accepted `build-paper-session-package` output can feed the accepted `paper-trade-session` runtime and report/display path under deterministic adapter doubles, without adding commands, changing CLI behavior, calling networks, adding credentials, entering live/order/private/account scope, calculating aggregate PnL, turning unknowns into zero, or creating second owner paths.
