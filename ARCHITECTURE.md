...
RX-061 is prepared as the next concrete local/manual/fake-money testing-support handoff: a CLI-layer display renderer for already-written RX-057 session report JSON artifacts. It must consume an explicit local session report path and produce deterministic local display output for later Telegram display adaptation while preserving report values exactly, including string-or-null economics and `aggregate_paper_net_profit_usd=null`. It must not run sessions, construct adapters, instantiate ledgers, mutate report/history results, write ledger events, call networks, add Telegram transport, touch live/order/private/account scope, replay/reconcile ledgers, migrate storage, add statuses/reasons, calculate aggregate PnL, or create second owner paths.
