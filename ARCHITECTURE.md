...
- Local paper session command payload fixture parsing, route-list validation for `paper-trade-session`, run command-text parsing for package-builder previews and parsers, and display command payload parsing for `render-paper-session-report` happen only in the CLI app layer, with `apps/cli/paper_session_payloads.py` owning side-effect-free payload normalization and validation helpers.
...
RX-055 adds that handoff as the explicit `paper-trade-session` CLI command in `apps/cli/main.py`. The command accepts only a non-empty finite local JSON array of at most 25 exact route objects and validates the whole route list before constructing any public adapter. Each route then flows serially through `run_real_data_research_route_with_snapshot()` and the shared `evaluate_route(route, snapshot, mode)` path in `EvaluationMode.ENTRY`, and fake paper handling delegates to `run_paper_lifecycle()` only when the retained public snapshot supplies a funding settlement timestamp. Session output is deterministic per-route stdout plus count-only summaries for outcomes, Entry EV known/unknown, paper expected funding known/unknown, paper total fees known/unknown, decision net profit known/unknown, and paper net profit known/unknown; it does not aggregate paper PnL and does not turn unknown economics into zero.

RX-056 records that the accepted RX-055 serial session outcome plus Product Owner and Control Tower direction clearly ground one next non-dangerous runtime handoff: a manual local JSON report/history export for `paper-trade-session` results.

RX-057 adds that handoff as the explicit `--session-report-json-path` option on the existing `paper-trade-session` CLI command. The export writes a local JSON report/history artifact only when the operator supplies that path. It is assembled after the existing serial session from already-validated route inputs, retained decisions/snapshots, paper lifecycle results, and paper ledger events. It preserves count-only known/unknown summaries, unknown/null semantics, the 25-route explicit ENTRY cap, and explicit non-aggregation of paper PnL. It does not add any second session runner, decision path, snapshot path, economics path, paper lifecycle path, ledger-write path, replay path, reconciliation path, execution-planning path, or live execution path.

RX-058 adds one app-local payload parser/fixture helper in `apps/cli/paper_session_payloads.py`. The helper parses explicit local JSON payload fixtures into the exact route-list dictionary shape already accepted by `paper-trade-session --routes-json-path`, and the existing file-path loader now delegates to the same validation boundary. It preserves the RX-055 25-route explicit ENTRY cap, exact route fields, timezone-aware assembled timestamp requirement, known/unknown/null behavior by omitting economics/report values, and fail-closed malformed input behavior before any adapter construction. It does not run sessions, construct adapters, write ledgers, write report artifacts, call networks, send messages, add credentials, discover/rank/watchlist/poll/schedule, replay or reconcile ledgers, plan execution, place orders, mutate eligibility, add statuses/reasons, or create second owner paths.

RX-059 is governance/source-of-truth only. It records explicit Product Owner direction that the accepted RX-055 manual serial session runner, RX-057 local report/history export, and RX-058 local payload parser together ground one next non-dangerous testing-support handoff: a local operator-package/preview builder for serial paper sessions. RX-060 must stay in the CLI app layer, consume the RX-058 parser/validation boundary, write only explicit local package artifacts, and stop before session execution, adapter construction, ledger writes, session report/history results, Telegram transport, network behavior, live/order/private/account scope, replay/reconciliation/storage migration, statuses/reasons, aggregate PnL, unknown-to-zero behavior, or second owner paths.

RX-060 adds that handoff as the explicit `build-paper-session-package` CLI command in `apps/cli/main.py`. The command reads an operator-supplied local command payload fixture path, validates the full payload through `apps/cli/paper_session_payloads.py`, then writes only explicit local output paths: one route-list JSON artifact accepted by `paper-trade-session --routes-json-path` and one descriptive preview/manifest JSON. The manifest records route count, route ids, intended route-list and session-report paths, and the exact manual `paper-trade-session --routes-json-path ... --session-report-json-path ...` command plan. It does not run `paper-trade-session`, construct adapters, instantiate ledgers, call fake paper lifecycle, write report/history results, write ledger events, call networks, add Telegram transport, touch live/order/private/account scope, replay/reconcile ledgers, migrate storage, add statuses/reasons, invent aggregate PnL, or create second owner paths.

RX-061 adds that handoff as the explicit `render-paper-session-report` CLI command in `apps/cli/main.py`. The command reads only an explicit local RX-057 session report JSON path, validates the accepted report shape, and prints deterministic stdout display lines copied from the report: route count, route ids, per-route decision status, per-route paper started state, string-or-null economics, known/unknown summary counts, and `aggregate_paper_net_profit_usd=null`. It does not run sessions, construct adapters, instantiate ledgers, mutate report/history results, write ledger events, call networks, add Telegram transport, touch live/order/private/account scope, replay/reconcile ledgers, migrate storage, add statuses/reasons, calculate aggregate PnL, or create second owner paths.

RX-062 adds that handoff as a side-effect-free display command payload parser in `apps/cli/paper_session_payloads.py` plus the explicit `render-paper-session-report-from-payload` CLI command in `apps/cli/main.py`. The parser accepts only explicit local JSON payload fixtures with exactly `schema_version=1` and `session_report_json_path`, returns only the normalized local report path, and does not read report JSON, render output, write artifacts, or invent defaults. The command reads an explicit payload fixture path, validates the payload before report reading, and delegates to the accepted RX-061 renderer for copied stdout display. It does not run sessions, construct adapters, instantiate ledgers, write ledger events, write or mutate report/history artifacts, call networks, add Telegram transport, touch live/order/private/account scope, replay/reconcile ledgers, migrate storage, add statuses/reasons, calculate aggregate PnL, turn unknowns into zero, or create second owner paths.

RX-063 adds `build-paper-session-display-payload` as the next concrete local/manual/fake-money testing-support handoff after RX-062 finalization. The command consumes one explicit already-written RX-057 session report JSON path and one explicit display payload JSON output path, validates the report through the accepted RX-061 display validation without printing display output, validates the generated payload through the accepted RX-062 parser, writes only `schema_version=1` and `session_report_json_path`, and prints deterministic local path summary lines. It remains display-preparation-only, without Telegram transport, credentials, messaging/network behavior, session execution, adapter construction, ledger writes, report/result mutation outside the new payload fixture artifact, execution automation, discovery/ranking/polling, live/order/private/account scope, aggregate PnL calculation, unknown-to-zero behavior, or second owner paths.

RX-064 adds `build-paper-session-display-command-preview` as the next concrete local/manual/fake-money testing-support handoff after RX-063 finalization. The command consumes one explicit local RX-062 display payload fixture path and one explicit local preview/manifest JSON output path, validates the display payload fixture through the accepted RX-062 parser without reading or rendering the referenced report JSON, and writes one descriptive preview/manifest artifact with only the display payload path and the exact manual `render-paper-session-report-from-payload --paper-session-display-command-payload-json-path ...` command plan. It remains display-command-preparation-only, without Telegram transport, credentials, messaging/network behavior, session execution, adapter construction, ledger writes, report/result mutation, execution automation, discovery/ranking/polling, live/order/private/account scope, aggregate PnL calculation, unknown-to-zero behavior, or second owner paths.

RX-065 adds `parse-paper-session-display-command-text` as the next concrete local/manual/fake-money testing-support handoff after RX-064 finalization. The command consumes one explicit local command text fixture path and one explicit local display payload JSON output path, accepts exactly `paper-session-report-display --session-report-json-path <session-report-json-path>` using `shlex.split()`, validates the generated fixture through the accepted RX-062 parser, and writes exactly the minimal RX-062 display payload shape. It remains command-text parsing and display-payload preparation only, without Telegram transport, credentials, messaging/network behavior, report rendering, report JSON reading, session execution, adapter construction, ledger writes, report/result mutation, execution automation, discovery/ranking/polling, live/order/private/account scope, aggregate PnL calculation, unknown-to-zero behavior, or second owner paths.

RX-066 adds `build-paper-session-display-command-text-preview` as the next concrete local/manual/fake-money testing-support handoff after RX-065 finalization. The command consumes one explicit local command text fixture path, one explicit intended display payload JSON path, and one explicit preview/manifest JSON output path, validates command text through the accepted RX-065 parser, validates the generated payload through the accepted RX-062 display payload boundary, and writes only a descriptive local manifest for the exact manual `parse-paper-session-display-command-text --paper-session-display-command-text-path ... --display-payload-json-path ...` command plan using `shlex.join()`. It remains command-text preview only, writes no display payload artifact, reads or renders no report JSON, and avoids session execution, adapter construction, ledger writes, report/result mutation, messaging/network behavior, execution automation, discovery/ranking/polling, live/order/private/account scope, aggregate PnL calculation, unknown-to-zero behavior, or second owner paths.

RX-067 adds deterministic local smoke fixture coverage for the existing `paper-trade-session` runtime path. The coverage is test-only and uses injected public-adapter doubles for two explicit valid `ENTRY` routes so the accepted serial flow reaches `run_real_data_research_route_with_snapshot()`, `assemble_route_snapshot_from_adapters()`, `evaluate_route()`, `run_paper_lifecycle()`, `core/accounting/ledger.py`, optional local SQLite persistence, deterministic stdout, and explicit `--session-report-json-path` export without external public network calls. The coverage verifies one started fake paper lifecycle and one valid unknown-economics route rejected through the existing lifecycle/ledger path, string-or-null economics, known/unknown count semantics, `aggregate_paper_net_profit_usd=null`, no aggregate paper PnL calculation, and no unknown-to-zero behavior. RX-067 adds no production code, user-facing command, CLI behavior change, adapter endpoint change, Telegram transport, credential handling, live/order/private/account scope, or second owner path.

RX-068 adds deterministic local smoke fixture coverage connecting accepted `build-paper-session-package` output to the accepted `paper-trade-session` runtime and report/display path. The coverage is test-only and uses the existing package command to write an explicit local command-payload-derived route-list artifact plus preview/manifest, then feeds that generated route-list artifact into the existing serial session runtime under injected public-adapter doubles with an explicit local SQLite ledger path and `--session-report-json-path` export. The produced report is validated through the accepted `render-paper-session-report` path. RX-068 verifies deterministic package preview values, accepted route-list output shape, existing fake paper lifecycle handling, ledger event counts/sequences/types, string-or-null economics, known/unknown count semantics, `aggregate_paper_net_profit_usd=null`, no aggregate paper PnL calculation, and no unknown-to-zero behavior. RX-068 adds no production code, user-facing command, CLI behavior change, adapter endpoint change, Telegram transport, credential handling, live/order/private/account scope, or second owner path.

RX-069 adds deterministic local smoke fixture coverage connecting accepted `build-paper-session-package`, `paper-trade-session`, `build-paper-session-display-payload`, `build-paper-session-display-command-preview`, `build-paper-session-display-command-text-preview`, `parse-paper-session-display-command-text`, and `render-paper-session-report-from-payload` command paths end-to-end. The coverage is test-only and uses an explicit local command payload fixture, generated route-list artifact, explicit local SQLite ledger path, explicit report export, generated display payload, display command preview, exact local command text fixture, command-text preview, parsed display payload, and payload-backed render under injected public-adapter doubles. RX-069 verifies deterministic local artifacts/stdout, one known-economics fake paper lifecycle, one unknown-economics rejection, ledger event counts/sequences/types, string-or-null economics, known/unknown count semantics, `aggregate_paper_net_profit_usd=null`, no aggregate paper PnL calculation, and no unknown-to-zero behavior. RX-069 adds no production code, user-facing command, CLI behavior change, adapter endpoint change, Telegram transport, credential handling, live/order/private/account scope, aggregate PnL calculation, unknown-to-zero behavior, or second owner path.

RX-070 adds focused deterministic local smoke fixture coverage over malformed or unsafe local operator/display fixture boundaries in the accepted command chain. It reuses accepted commands without adding new commands, production behavior, Telegram transport, network or credentials, live/order/private/account scope, aggregate PnL calculation, unknown-to-zero behavior, or second owner paths. After RX-070 finalization, RX-071 is the next handoff as a narrow governance/source-of-truth clarification gate before any further command-interface work is selected.

RX-071 is governance/source-of-truth only. It records that the accepted package/runtime/report/display chain plus RX-070 fail-closed coverage ground exactly one next non-dangerous local/manual/fake-money command-interface handoff: a local paper-session run command text preview builder. That later handoff may parse exact local run command text and validate referenced local paper-session command payload fixtures for preview only, but it must not run sessions, write package route-list artifacts, write package preview artifacts, write session reports, render displays, construct adapters, instantiate ledgers, call networks, send messages, add Telegram transport or credentials, automate execution, enter live/order/private/account scope, calculate aggregate PnL, turn unknowns into zero, or create second owner paths.

RX-072 adds that handoff as the explicit `build-paper-session-run-command-text-preview` CLI command plus one side-effect-free parser helper in `apps/cli/paper_session_payloads.py`. The parser accepts exactly `paper-session-run --paper-session-command-payload-json-path <payload-json-path> --routes-json-output-path <routes-json-output-path> --preview-json-output-path <package-preview-json-output-path> --session-report-json-path <session-report-json-path>` through `shlex.split()` and returns only the four local path strings needed by the accepted package builder. The command reads only the command text fixture, parses the referenced command payload fixture path, rejects normalized local path collisions across those two fixture paths, the preview output path, and every referenced intended package/session output path before reading the payload fixture or writing any artifact, then validates the payload through the accepted RX-058 route-list boundary and writes only one descriptive preview/manifest for the exact manual `build-paper-session-package ...` command plan. It does not run sessions, write route-list/package-preview/session-report artifacts, render displays, construct adapters, instantiate ledgers, call networks, send messages, add Telegram transport or credentials, automate execution, enter live/order/private/account scope, calculate aggregate PnL, turn unknowns into zero, or create second owner paths.

RX-073 adds `parse-paper-session-run-command-text` in `apps/cli/main.py` as the local parser counterpart to RX-072. It reuses the accepted RX-072 exact command-text helper and RX-058 payload validation boundary, rejects normalized local path collisions across the command text fixture, payload fixture, referenced route-list output, referenced package-preview output, and intended session-report path before payload reads or writes, then writes only the accepted RX-060 route-list artifact and package preview/manifest artifact. It does not invoke `build-paper-session-package`, run `paper-trade-session`, write session reports, render displays, construct adapters, instantiate ledgers, call networks, send messages, add Telegram transport or credentials, automate execution, enter live/order/private/account scope, calculate aggregate PnL, turn unknowns into zero, or create second owner paths.

RX-074 adds test-only deterministic smoke fixture coverage for the accepted run-command-text parser output feeding the accepted fake-money serial runtime/report/display path. The coverage begins with an explicit local command payload fixture and exact local `paper-session-run ...` command text, runs only `parse-paper-session-run-command-text` for package artifact generation, verifies that parsing reads/writes only accepted local parser/package artifacts and does not write the intended session report or ledger, then feeds the generated route-list artifact into `paper-trade-session` under injected deterministic public-adapter doubles and an explicit local SQLite ledger path. It validates the resulting report through the accepted display command while preserving the single decision path, paper lifecycle owner, ledger owner, report owner, display owner, string-or-null economics, known/unknown count semantics, `aggregate_paper_net_profit_usd=null`, no aggregate paper PnL calculation, and no unknown-to-zero behavior. RX-074 adds no production code, user-facing command, CLI behavior change, parser weakening, network/credential/Telegram/live/order/private/account behavior, or second owner path.
