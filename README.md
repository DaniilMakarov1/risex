...
RX-059 records explicit Product Owner direction that the next needed safe step toward a fuller fake-money paper trader system for serial strategy testing is a local-only operator-package/preview builder. RX-060 must consume explicit local command payload fixtures through the RX-058 parser/validation boundary and produce deterministic local operator artifacts for manual testing and later Telegram display adaptation, such as a validated route-list JSON file and a preview/manifest JSON describing route count, route ids, intended local input/report paths, and the exact manual `paper-trade-session --routes-json-path ... --session-report-json-path ...` command plan. RX-060 must not execute sessions, construct adapters, write ledger events, write session report/history results, send messages, call networks, add Telegram transport, credentials, alerts, discovery, polling, execution automation, live/order/private/account behavior, replay, reconciliation, storage migrations, new statuses/reasons, aggregate PnL, unknown-to-zero behavior, or second owner paths.
