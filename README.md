...
RX-047 records that the next safe reporting handoff is RX-048: an opt-in structured JSON stdout form of the same manual one-route public readiness report. RX-048 must reuse the existing public adapters, one-route adapter handoff, retained snapshot/report helper, source-aware fee/funding completion, and `evaluate_route()` path; RX-047 does not implement that output.
