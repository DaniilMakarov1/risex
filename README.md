...
After RX-060, the next concrete fake-money paper-trader testing-support handoff is RX-061 Local Paper Session Report Display Renderer. RX-061 must consume already-written RX-057 session report JSON from an explicit local path and produce deterministic local display output suitable for later Telegram display adaptation. It may display route count, route ids, per-route decision/paper statuses already present in the report, known/unknown summary counts, string-or-null economics exactly as represented in the report, and `aggregate_paper_net_profit_usd` preserved as null/unknown. RX-061 must not run sessions, construct adapters, write ledgers, mutate reports, send messages, call networks, add Telegram transport, credentials, discovery/ranking/polling, execution automation, live/order/private/account behavior, replay, reconciliation, storage migrations, new statuses/reasons, aggregate PnL calculation, unknown-to-zero behavior, or second owner paths.
